...
# --- PER-ATHLETE FILE PERSISTENCE ---
def get_user_store_filename() -> str:
    creds = st.session_state.get("user_credentials") or {}
    athlete_id = creds.get("icu_id") or "default_athlete"
    safe_id = re.sub(r"[^\w\-]", "_", athlete_id)
    return f"athlete_store_{safe_id}.json"

def load_disk_store() -> Dict[str, Any]:
    filename = get_user_store_filename()
...
def save_disk_store():
    ...
    if localS:
        creds = st.session_state.get("user_credentials") or {}
        prefix = creds.get("icu_id", "default")
        for k, v in store.items():
            try:
                localS.setItem(f"athlete_{prefix}_{k}", v)
            except Exception:
                pass
...
# --- CLEAN CHAT CONTENT & EXTRACT WORKOUTS ---
def clean_chat_content(text: str) -> str:
    if not text: return ""
    cleaned = re.sub(r"```xml\s*<\?xml.*?</workout_file>\s*```", "", text, flags=re.S | re.I)
    cleaned = re.sub(r"```(?:json:workouts|json|workouts)?\s*\[\s*\{.*?\}(?:\s*,\s*\{.*?\})*\s*\]\s*```", "", cleaned, flags=re.S | re.I)
...
def extract_json_workouts(text: str) -> List[Dict[str, Any]]:
    if not text: return []
    patterns = [
        r"```(?:json:workouts|json|workouts)\s*(\[\s*\{.*?\}\s*\])\s*```",
        r"```\s*(\[\s*\{.*?\}\s*\])\s*```",
        r"(\[\s*\{\s*\"(?:date|title|name)\".*?\}\s*\])"
    ]
    for pat in patterns:
...
CRITICAL WORKOUT SYNC MANDATE:
Whenever you prescribe, recommend, or adjust ANY workout sessions or plans, ALWAYS attach a structured JSON block at the very end of your response inside ```json:workouts ... ```.

The `description` field MUST follow native Intervals.icu plain-text workout syntax so MyWhoosh can read and execute the workout:
...
JSON Structure to append at the end of response:
```json:workouts
[
  {{
    "date": "2026-09-05",
...
Do NOT show code blocks or syntax text in your standard conversational prose. Explain the workout in clean text, then put the sync payload at the end inside ```json:workouts."""
...
st.markdown(f"##### 🤖 AI Multi-Sport Coach ({st.session_state.active_session_id})", unsafe_allow_html=True)
