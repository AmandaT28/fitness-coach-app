...
AI_TIMEOUT = 15  # Snappy timeout to keep UI responsive
INTERVALS_TIMEOUT = 6
...
def ensure_initial_message():
    if not st.session_state.messages:
        st.session_state.messages = [{"role": "assistant", "content": "Hello! I am your AI Performance Coach. Ask me to draft next week's training block or adjust your current schedule."}]
...
def coach_prompt(question, display_name):
    ...
    return f"""You are an elite cycling coach.
...
INSTRUCTIONS:
1. Keep replies concise and tactical.
2. IF PRESCRIBING A WEEKLY PLAN, append a JSON array in `<icu_weekly_plan>` tags:
<icu_weekly_plan>
[
  {{
    "name": "VO2 Max 5x3min",
...
3. IF PRESCRIBING A SINGLE WORKOUT, enclose in `<icu_workout>` tags."""
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if isinstance(icu_payload, list) and len(icu_payload) > 0:
                ...
                if st.button("🚀 Sync Plan to Intervals.icu & MyWhoosh", key=f"sync_chat_{len(st.session_state.messages)}", type="primary"):
                    try:
...
if selected_nav == NAV_OPTIONS[0]:
    ...
    if uploaded:
        ...
        if metrics:
            ...
            st.error("Could not parse GPX file. Ensure it contains valid track points.")
