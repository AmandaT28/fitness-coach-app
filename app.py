...
if st.session_state.active_nav == NAV_OPTIONS[0]:
    ...
    <div style="background:{BG_CARD}; border:1px solid {card_border}; border-radius:10px; padding:18px; margin-bottom:20px;">
        ...
        <p style="margin:4px 0 0 0; font-size:0.85rem; color:{TEXT_MUTED};">ACWR: {acwr} ({acwr_status}) | TSB: {tsb:.1f} | Sleep: {int(sleep) if sleep > 0 else 'N/A'}/100 | HRV: {int(hrv) if hrv > 0 else 'N/A'}ms | RHR: {int(rhr) if rhr > 0 else 'N/A'}bpm</p>
    </div>
    """, unsafe_allow_html=True)
...
# VIEW 3: TRAINING CALENDAR
    ...
    with st.expander("📝 Log Sickness, Travel, or Unavailability", expanded=False):
        ...
        if st.session_state.protected_events:
            st.markdown("###### Currently Logged Life Events & Unavailability")
