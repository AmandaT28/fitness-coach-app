...
def build_gemini_payload(current_question, wellness_list, gpx_content: Optional[str] = None):
    ...
    protected_events_list = st.session_state.get('protected_events', [])
    protected_ctx = f"\nACTIVE TRAVEL / LIFE / SICKNESS EVENTS (SCHEDULE AROUND THESE):\n{json.dumps(protected_events_list, ensure_ascii=False)}" if protected_events_list else ""
    ...
    system_instructions = (
        ...
        f"{gpx_injection}\n\n"
        "MANDATORY INTERVALS.ICU WORKOUT BUILDER SYNTAX RULES (FOR MYWHOOSH SYNC):\n"
        "All structured workout descriptions inside JSON payloads or text must strictly follow Intervals.icu syntax rules:\n"
        "1) Basic line format: - [duration/distance] [target] [optional cadence]\n"
        "   - Durations: Use 'm' for minutes, 's' for seconds, 'h' for hours (e.g., 10m, 30s, 1h2m30s). For meters use 'mtr', kilometers use 'km'.\n"
        "   - Targets: Power FTP percentage (e.g., 75%, 95-105%), absolute watts (e.g., 220w), zones (e.g., Z2). Heart rate (e.g., 70% HR, Z2 HR).\n"
        "   - Cadence (cycling): Append cadence after target (e.g., - 10m 75% 90rpm, - 12m 85% 90-100rpm).\n"
        "2) Ramps & Freeride: Use 'ramp' for gradual changes (e.g., - 10m ramp 50%-75% or - 15m ramp 60%-90% 85rpm). Use '20m freeride' for ERG off.\n"
        "3) Repeats: Place repeat header on its own line followed by 'x' (e.g., Main Set 4x or 5x), leaving one empty line before and after repeat blocks. Nesting is not supported.\n"
        "   Example Repeat Block:\n"
        "   Main Set 4x\n"
        "   - 2m 95%\n"
        "   - 2m 55%\n"
        "4) Text Prompts & Cues: Any text before the first duration becomes a cue (e.g., Warmup 10m 60%).\n"
        "MANDATORY: IF PRESCRIBING WORKOUTS FOR CALENDAR SYNC, ALWAYS INCLUDE A VALID JSON ARRAY inside <icu_weekly_plan> tags matching the required structure, ensuring the 'description' field uses these exact syntax rules."
    )
